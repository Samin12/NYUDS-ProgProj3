  ...
  // comp.compare(target, i) == 0; otherwise, returns false.
 {
  ...
  // Enqueues the elements from the subtree rooted at node into q in postOrder.  
  {
    ...
    public void printTreeByLevel() {
        ...
        treeArray[currentLevelNumber][50] = root.getInfo().toString();


        while (!currentLevel.isEmpty()) {
            Iterator<BSTNode> it = currentLevel.iterator();
            while (it.hasNext()) {
                BSTNode currentNode = it.next();
                if (currentNode.getLeft() != null) {
                    treeArray[currentLevelNumber+1][columNumber-1]="/";

                    currentLevelNumber=currentLevelNumber+2;
                    columNumber=columNumber/2;

                    nextLevel.add(currentNode.getLeft());
                    treeArray[currentLevelNumber][columNumber]=currentNode.getLeft().getInfo().toString();

                }
                if (currentNode.getRight() != null) {
                    treeArray[currentLevelNumber+1][columNumber+1]="\\";
                    currentLevelNumber=currentLevelNumber+2;
                    columNumber=columNumber+(columNumber/2);
                    nextLevel.add(currentNode.getRight());
                    treeArray[currentLevelNumber][columNumber]=currentNode.getRight().getInfo().toString();
